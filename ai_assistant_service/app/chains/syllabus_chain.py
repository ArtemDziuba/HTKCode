...
def run_syllabus_detection(text: str, api_key: str) -> dict:
    ...
    try:
        llm = get_llm(api_key, temperature=0.1).bind(response_format={"type": "json_object"})
        chain = ChatPromptTemplate.from_messages([
            ("system", SYLLABUS_SYSTEM),
            ("human", "{document}"),
        ]) | llm

        logger.info("[syllabus] calling LLM on %d chars...", len(truncated))
        raw = chain.invoke({"document": truncated})
        content = raw.content if hasattr(raw, "content") else str(raw)
        parsed = json.loads(content)
        if not isinstance(parsed, dict):
            logger.warning("[syllabus] unexpected response shape, returning empty")
            return _EMPTY
        ...
        if result["is_syllabus"]:
            logger.info(
                "[syllabus] ✅ DETECTED — course=%r weeks=%d",
                result["course_name"], len(result["weeks"]),
            )
        else:
            logger.info("[syllabus] not a syllabus")
        return result
    except Exception as exc:
        logger.exception("[syllabus] ERROR: %s", exc)
        return _EMPTY
