...
def run_section_match(
    ...
    try:
        logger.info("[section_match] matching %r against %d weeks", filename, len(week_titles))
        ...
        section = section if 1 <= section <= len(week_titles) else 0
        logger.info("[section_match] %r → section %d", filename, section)
        return section
    except Exception as exc:
        logger.exception("[section_match] ERROR: %s", exc)
        return 0
